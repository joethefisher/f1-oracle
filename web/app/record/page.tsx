...
import { MOCK_SEASON_RECORDS } from "@/lib/mockData";
...
export default function RecordPage() {
  const [open, setOpen] = useState<Set<number>>(new Set([MOCK_SEASON_RECORDS[0]?.race.id]));
  ...
  const totalBets = MOCK_SEASON_RECORDS.reduce((s, r) => s + r.n_bets, 0);
  const totalWins = MOCK_SEASON_RECORDS.reduce((s, r) => s + r.n_wins, 0);
  const totalPnl  = MOCK_SEASON_RECORDS.reduce((s, r) => s + r.total_pnl, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {MOCK_SEASON_RECORDS.map((record) => {
            ...
            <div key={record.race.id} className="rounded-lg border border-zinc-800 overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-zinc-500 text-sm">{record.race.race_date_utc.slice(0, 10)}</span>
                </div>
      ...
      <p className="text-xs text-zinc-600">
        Data shown is mock/demo data. Connect Supabase to show live season record.
      </p>
    </div>
  );
}
